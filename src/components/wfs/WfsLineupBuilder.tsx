...
/** The 12 WFS lineup slots: every GICS sector plus Crypto. */
const WFS_SECTORS = [
  ...DRAFT_POOL_SECTORS.filter((s) => s !== "All"),
  "Crypto",
] as const;

type WfsSector = (typeof WFS_SECTORS)[number];
...
export function WfsLineupBuilder({ contestId }: { contestId: string }) {
  ...
  const [picks, setPicks] = useState<Partial<Record<WfsSector, WfsPick>>>({});
  ...
  function selectPick(sector: WfsSector, quote: MarketQuote, name: string) {
    setPicks((prev) => {
      const next = {
        ...prev,
        [sector]: {
      ...
      const nextUnfilled = WFS_SECTORS.find((s) => !next[s]);
  ...
  const lineupComplete = filledCount === WFS_SECTORS.length;
  ...
  async function submitLineup() {
    ...
    try {
      const response = await fetch("/api/sdwfs/enter", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          contestId,
          picks: WFS_SECTORS.map((sector) => ({
            sector,
