...
export default async function WfsResultsPage({
  params,
}: {
  ...
  const { data: contest, error: contestError } = await supabase
    .from("sdwfs_contests")
    .select("id, week_start_date, buy_in, status")
    .eq("id", params.contestId)
    .maybeSingle();

  if (contestError || !contest) {
    return (
      <WfsShell title="SDWFS — Results">
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        <div>
          ...
          <p className="text-muted text-sm mt-1">
            Week of {contest.week_start_date} · ${contest.buy_in} buy-in · Prize pool: ${prizePool.toFixed(2)}
          </p>
          ...
          <div className="space-y-2">
            ...
            {entries.map((entry) => {
              ...
              return (
                <Link
                  key={entry.id}
                  href={`/stockdraft-wfs/entry/${entry.id}`}
