...
import {
  hasSdhl2026PickInjuryMapForLeague,
  lookupSdhl2026PickInjuryMapForSymbol,
} from "@/lib/sim/sdhl-2026-pick-injury-map";
import { lookupPlayerIdForSymbol } from "@/lib/sim/stock-player-map";
import type { IrEligibilityResult } from "@/lib/sim/types";

// SDHL 2026 season only. Mirrors sdfl-2026-injury-status.ts. Reuses the
// shared week/date-window math from injury-status.ts, but resolves symbol ->
// player via the 2026 rank data (sim_stock_player_map for S&P symbols,
// sim_sdhl_2026_pick_injury_map for everything else) instead of the 2024
// beta trial's cycle-of-100 map. NHL is date-windowed, not week-windowed
// (simSportUsesWeekNumbers("nhl") is false), so resolveSimEligibilityContext
// resolves this to date mode automatically — no week_offset shifting needed
// the way SDFL's fallback pool uses it.
...
export async function isSdhl2026StockIrEligible(
  supabase: SupabaseClient,
  leagueId: string,
  symbol: string,
  ...
  const fallbackRow = await lookupSdhl2026PickInjuryMapForSymbol(supabase, leagueId, upper);
  if (fallbackRow) {
    ...
    if (map.mapMissing) {
      return {
        eligible: false,
        error: `Stock-to-player mapping is not seeded for SDHL ${SEASON}.`,
      };
    }
    if (!map.playerId) {
      return {
        eligible: false,
        error: `${upper} has no mapped player for SDHL ${SEASON}.`,
      };
    }
...
/**
 * Injured symbols for a whole SDHL 2026 roster in one shot. Mirrors
 ...
 */
export async function loadSdhl2026InjuredSymbolsForLeague(
  supabase: SupabaseClient,
  leagueId: string,
  symbols: string[],
  leagueWeekNumber: number,
