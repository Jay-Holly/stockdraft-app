...
import { getAnchors } from "@/lib/pricing/read";
import {
  computePickMarketValue,
  isTrustworthyValue,
  isPlausibleMove,
} from "@/lib/scoring/values";
...
export async function captureClosingValues(
  ...
  // Picks with no prior row need a real opening price for this session, not
  // the close reused.
  //
  // This used to call Twelve Data directly, rationed to that provider's
  // per-minute credit budget and limited to equities (a crypto ticker resolves
  // to a different asset there). It now reads the session's open anchor out of
  // the price log, which is strictly better in three ways: the logger has
  // already fetched and corroborated that number, there is no credit budget to
  // ration so every pick can be covered in one pass, and crypto is included
  // because the log is keyed by symbol and does not care about asset class.
  //
  // The rule it enforces is unchanged: get the real number, or leave the pick
  // uncaptured for a later sweep. Never seed it from the close.
  const needsRealOpen = entries.filter(
    ({ pick, closePrice }) => closePrice !== null && !existingByPick.has(pick.id)
  );
  const openSymbols = [
  ...
  const openAnchors =
    openSymbols.length > 0
      ? await getAnchors(openSymbols, sessionDateIso, "open")
      : null;
  const historicalOpens: Record<string, { open: number } | undefined> = {};
  if (openAnchors) {
    for (const [symbol, hit] of openAnchors.hits) {
      historicalOpens[symbol] = { open: hit.price };
    }
  }
